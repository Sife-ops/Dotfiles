...
case $chosen in
    ...
    "focus desktop") bspc desktop -f $(dmenudesk.sh) ;;
    "focus parent") bspc node @parent -f ;;
    "focus window") bspc node $(dmenuwin.sh) -f ;;
    "kill all windows") bspc node any.local -c ;;
    "kill window") bspc node "$(dmenuwin.sh -f)" -c ;;
    "kill other desktops")
    ...
    "kill other windows") bspc node @/ -i
        ...
        bspc node @/1 -c ;;
    "kill desktop") bspc desktop $(dmenudesk.sh -f) -r ;;
    "move to monitor") bspc desktop focused -m $(dmenumon.sh) ;;
    "move to desktop") bspc node -d $(dmenudesk.sh) ;;
    ...
    "swap desktop") bspc desktop -s $(dmenudesk.sh) ;;
